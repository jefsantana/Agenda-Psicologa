...
- Prontuário: abas Evolução / Histórico / Anexos / Dados cadastrais. Evoluções versionadas (sem exclusão física — soft delete + log), com modelos SOAP e livre. Objetivos terapêuticos em checklist. Anexos (PDF/imagem) com limite de 20 MB. Selo "LGPD · sigiloso" visível; todo acesso gera registro em trilha de auditoria.
  - Dívida técnica conhecida (Fase 1): o registro de auditoria de *leitura* de prontuário é gravado pelo código do navegador e é não-bloqueante — se a rede falhar no momento da abertura, o acesso não fica registrado. A trilha em si é à prova de adulteração no banco (append-only, migração v9). Mover a gravação para o servidor (RPC/Edge Function que devolve o prontuário e grava o log na mesma chamada) fica para uma fase posterior.
