 ...
 * objeto `kpis` calculado na página; enquanto `carregando`, mostra reticências
  ...
  return (
    <KpiCard rotulo="Atendimentos hoje" icone={<IconeChecklist />}>
      <p className="kpi__valor">
        {carregando ? "…" : concluidos}
        {!carregando && <span className="kpi__valor-sec">/ {total}</span>}
      </p>
      <div className="kpi__progresso" aria-hidden="true">
        <span className="kpi__progresso-fill" style={{ width: `${pct}%` }} />
      </div>
      <p className="kpi__apoio">{carregando ? " " : legendaConcluidos(concluidos, total)}</p>
    </KpiCard>
  );
}
...
function KpiFaltas({ dados, carregando }) {
  ...
  return (
    <KpiCard rotulo="Faltas e remarcações" icone={<IconeAlerta />} tom={total > 0 ? "atencao" : undefined}>
      <p className="kpi__valor">
        {carregando ? "…" : total}
        {!carregando && total > 0 && <span className="kpi__tag">hoje</span>}
      </p>
      {!carregando &&
        (total > 0 ? (
          <>
